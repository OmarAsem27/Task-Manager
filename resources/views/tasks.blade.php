...
<body class="">
    ...
    <div class="container text-center bg-gray-400 max-w-full ">
        ...
        <div class="text-center bg-gray-200 ">
            @if ($tasks)
                @foreach ($tasks as $task)
                    <p> {{ $task->body }} -- {{ $task->status }} </p>
                    <div class="flex justify-center">
                        <form method="post" action="{{ route('tasks.destroy', $task) }}">
                            @csrf
                            @method('delete')
                            <button type="submit"
                                class="p-2 bg-red-700 hover:bg-red-800 rounded-xl text-white">Delete</button>
                        </form>
                        <form method="post" action="{{ route('tasks.update', $task) }}">
                            @csrf
                            @method('put')
                            <button type="submit"
                                class="p-2 bg-stone-700 hover:bg-stone-800 rounded-xl text-white">Edit</button>
                        </form>
                    </div>
                    <p>
        ...
        <form class="max-w-sm mx-auto" method="POST" action="{{ route('tasks.store') }}">
            @csrf
            <div class="mb-5">
                <label for="task" class="block mb-2 text-sm font-medium text-yellow-900 dark:text-red-500">Your
                    task</label>
            ...
            <div class="mb-5">
                <label for="category"
                    class="block mb-2 text-sm font-medium text-yellow-900 dark:text-red-500">Category</label>
